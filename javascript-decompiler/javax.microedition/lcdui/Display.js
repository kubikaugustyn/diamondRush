...
javax.microedition.lcdui.Display = class Display {// public class Display {
    constructor() {
        ...
        this.current = null// 	private Displayable current = null;
        this.accessor = null// 	private DisplayAccessor accessor = null;
        this.eventDispatcher = undefined// 	private EventDispatcher eventDispatcher;


        // 	Display() {
        this.accessor = new DisplayAccessor(this)// 		accessor = new DisplayAccessor(this);

        this.eventDispatcher = DeviceFactory.getDevice().getUIFactory().createEventDispatcher(this)// 		eventDispatcher = DeviceFactory.getDevice().getUIFactory().createEventDispatcher(this);

        this.timer.scheduleAtFixedRate(new RunnableWrapper(new TickerPaintTask()), 0, Ticker.PAINT_TIMEOUT)// 		timer.scheduleAtFixedRate(new RunnableWrapper(new TickerPaintTask()), 0, Ticker.PAINT_TIMEOUT);
        this.timer.scheduleAtFixedRate(new RunnableWrapper(new GaugePaintTask()), 0, Ticker.PAINT_TIMEOUT)// 		timer.scheduleAtFixedRate(new RunnableWrapper(new GaugePaintTask()), 0, Ticker.PAINT_TIMEOUT);
        // 	}
    }
...
// 		}
    ...
    getDisplay(...args) {// 		public Display getDisplay() {
        if (java_tools.areArgumentsInForm(args)) {
            return this.display// 			return display;
        }// 		}
        else if (java_tools.areArgumentsInForm(args, ["object"])) {//getDisplay() {// 	public static Display getDisplay(MIDlet m) {
            var result// 		Display result;

            if (MIDletBridge.getMIDletAccess(m).getDisplayAccess() == null) {// 		if (MIDletBridge.getMIDletAccess(m).getDisplayAccess() == null) {
                result = new Display()// 			result = new Display();
                MIDletBridge.getMIDletAccess(m).setDisplayAccess(result.accessor)// 			MIDletBridge.getMIDletAccess(m).setDisplayAccess(result.accessor);
            } else {// 		} else {
                result = MIDletBridge.getMIDletAccess(m).getDisplayAccess().getDisplay()// 			result = MIDletBridge.getMIDletAccess(m).getDisplayAccess().getDisplay();
            }// 		}
...
// 		}
    ...
    getCurrent() {// 		public Displayable getCurrent() {
        return this.getDisplay().getCurrent()// 			return getDisplay().getCurrent();
    }// 		}
...
// 		}
    ...
    setCurrent(d) {// 		public void setCurrent(Displayable d) {
        this.getDisplay().setCurrent(d)// 			getDisplay().setCurrent(d);
    }// 		}
